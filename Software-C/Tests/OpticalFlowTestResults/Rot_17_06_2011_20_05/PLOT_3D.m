...
for i=1:size(Files, 1)
     ...
     zData(idx_i,cnt_j)=sum((abs((Stimuli-(gain(idx_i, cnt_j).*Tfile(index,:))))/Stimuli));%mean((1-abs((Stimuli-(gain(idx_i).*Tfile(index,:))))/Stimuli));%var((gain(idx_i).*Tfile(index,1:80)), Stimuli(:,1:80));%std((gain(idx_i).*Tfile(index,:)));%
     hisData=[hisData;(Stimuli-(gain(idx_i, cnt_j).*Tfile(index,:)))'];
     idx=[idx_i, cnt_j]
     if(filesWidth==cnt_j)
       cnt_j=0;
     end
