...
void atdCalibrate(void)
{
 ...
 CopterState* mycopterState = mycopterGetStatePtr(); // For simulation purposes only
  ...
  // Set integratorstate for Gyroskope highpass filter to MATLAB simulation offset 
  mycopterState->r_Pitch_filt = 505; 
  mycopterState->r_Roll_filt = 505; 
  mycopterState->r_Yaw_filt = 505;
